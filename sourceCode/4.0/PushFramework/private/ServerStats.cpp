...
OutgoingPacket* ServerStats::getPerformancePacket()
{
	...
	ss << "<samplingRate val=\"" << pServerImpl->getServerOptions().samplingRate << "\"/>";
...
OutgoingPacket* ServerStats::getInitializationPacket()
{
    ...
    //Write header :
    ss << "<root typeId=\"";
    ss << AnalyticsProtocol::ServerInfoResponse;
    ss << "\">";
    ss << "<init>";
    ...
    ss << pServerImpl->getDispatcher().getServiceNames();
    ...
    ss << pServerImpl->getBroadcastManager().getQueuesNames();
