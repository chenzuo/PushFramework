...
bool DemuxImpl::start()
{
    ...
    int nThreads = pServerImpl->getServerOptions().nWorkersCount;
...
void DemuxImpl::proc(bool processReadEvents)
{
    ...
    while (!stopWorking)
    {
        int rval = pServerImpl->getIOQueue().getQueuedEvent(epoll_events, epollArraySize, processReadEvents);
        ...
        for (int i = 0; i < rval; i++)
        {
            ...
            if (events & EPOLLOUT)
            {
                pServerImpl->getDispatcher().handleWrite(*pPerSocketContext, -1);
            }
            if (events & EPOLLIN)
            {
                pServerImpl->getDispatcher().handleRead(*pPerSocketContext, -1);
            }
        }
    }
}
