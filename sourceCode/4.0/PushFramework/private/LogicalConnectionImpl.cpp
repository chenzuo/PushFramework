...
namespace PushFramework
{
	...
	{
		// Locate the queue.
		BroadcastQueue* pQueue = getServerImpl().getBroadcastManager().getQueueWithLock(queueName);
		if (!pQueue)
		{
			return false;
		}

        SubscribeToQueue(pQueue, ignorePreviousPackets);		

        getServerImpl().getBroadcastManager().releaseReadLock();
		return true;
	}

    void LogicalConnectionImpl::SubscribeToQueue(BroadcastQueue* pQueue, bool ignorePreviousPackets /*= false*/)
    {
