...
DemuxImpl::DemuxImpl(ServerImpl* pServerImpl)
{
...
bool DemuxImpl::start()
{
    ...
    for ( unsigned int i = 0; i < pServerImpl->getServerOptions().nWorkersCount; i++ )
    {
...
void DemuxImpl::stop()
{
	...
	for (size_t i = 0; i < workersThreadsVect.size(); i++)
	{
		...
		pServerImpl->getIOQueue().postTerminationSignal();
	}
...
void DemuxImpl::proc()
{
    ...
    {
        BOOL bSuccess = pServerImpl->getIOQueue().getQueuedEvent(&dwIoSize,(ULONG_PTR*) &pConnection,&lpOverlapped);
        ...
        if(!bSuccess)
        {
			if (pConnection)
			{
				...
				pServerImpl->getDispatcher().handleFailedIO(*pConnection);
			}
            continue;
        }
        if (pConnection==NULL)
        ...
        {
        case IORead:
            pServerImpl->getDispatcher().handleRead(*pConnection,dwIoSize);
            break;
        case IOWrite:
            pServerImpl->getDispatcher().handleWrite(*pConnection,dwIoSize);
            break;
        case IOInitialize:
            pServerImpl->getDispatcher().handleInitialize(*pConnection);
