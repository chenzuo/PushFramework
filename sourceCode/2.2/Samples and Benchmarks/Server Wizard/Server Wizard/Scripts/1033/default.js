...
function AddFilters(proj)
{
	try
	{
		...
		var group = proj.Object.AddFilter('Proto');
		group.Filter = "proto";
	}
	catch(e)
	{
...
function AddConfig(proj, strProjectName) {
	...
	try
	{
		...
		var pushIncludes = wizard.FindSymbol('PUSHFRAMEWORK_PATH') + '\\include';
		var protobufIncludes = wizard.FindSymbol('PROTOBUF_PATH') + '\\src\\';

		CLTool.AdditionalIncludeDirectories = '"' + pushIncludes + '";"' + protobufIncludes + '"';

		var LinkTool = config.Tools('VCLinkerTool');

		var pushLib = wizard.FindSymbol('PUSHFRAMEWORK_PATH') + '\\$(ConfigurationName)';
		var protobufLib = wizard.FindSymbol('PROTOBUF_PATH') + '\\vsprojects\\$(ConfigurationName)';

		LinkTool.AdditionalLibraryDirectories = '"' + pushLib + '";"' + protobufLib + '"';
		LinkTool.AdditionalDependencies = "libprotobuf.lib pushframework.lib";
...
function GetTargetName(strName, strProjectName)
{
	try
	{
	    ...
	    if (strName == 'messages.proto')
	        strTarget = 'proto\\messages.proto';

	    if (strName == 'messages.pb.h')
	        strTarget = 'generated\\messages.pb.h';

	    if (strName == 'messages.pb.cc')
	        strTarget = 'generated\\messages.pb.cc';
