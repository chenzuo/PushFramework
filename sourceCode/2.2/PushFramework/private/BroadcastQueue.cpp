...
BroadcastQueue::BroadcastQueue(BroadcastManagerImplBase* pManager, std::string channelName, QueueOptions& queueOptions)
{
...
void BroadcastQueue::disposePacket( PacketInfo* pPacketInfo, std::string clientKey, bool bSuccess /*= true*/ )
{
    ...
    if(bSuccess)
        clientMap[clientKey] = pPacketInfo->packetId;
...
OutgoingPacket* BroadcastQueue::getNextPacket( std::string clientKey, PacketInfo*& pPacketInfo )
{
    ...
    if(cIt == clientMap.end() && queueOptions.requireRegistration)
        return NULL;//user not subscribed to this channel.

	if (queueOptions.ignorePreviousPackets && cIt == clientMap.end())
	{
		//If client has no context, the set it to previously generated id and return NULL.
		clientMap[clientKey] = lastGenId;
		return NULL;
	}
