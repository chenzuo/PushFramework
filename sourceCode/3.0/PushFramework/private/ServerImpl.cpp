...
void ServerImpl::stop()
{
	...
	//Free memory pools:
	thePhysicalConnectionPool.deleteAllocatedMemory();
	logicalConnectionPool.deleteAllocatedMemory();
	connectionContextPool.deleteAllocatedMemory();
...
unsigned int ServerImpl::calculateAdditionalBuffersForProtocols(unsigned int nMaxPoolConnections , unsigned int nMaxThreads)
{
	...
	while (it != listenersMap.end())
	{
		...
		while (pProtocol)
		{
			nTotal += pProtocol->getRequiredRecyclableBuffers(nMaxPoolConnections, nMaxThreads);
			pProtocol = pProtocol->getLowerProtocol();
		}
...
bool ServerImpl::initializeProtocolContextPools( unsigned int nMaxPoolConnections )
{
	...
	while (it != listenersMap.end())
	{
		...
		while (pProtocol)
		{
			if (!pProtocol->initialize(nMaxPoolConnections))
				return false;
			pProtocol = pProtocol->getLowerProtocol();
		}
