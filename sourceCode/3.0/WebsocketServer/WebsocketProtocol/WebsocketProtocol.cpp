...
DecodeResult::Type WebsocketProtocol::tryDecode( ProtocolContext& context, Buffer& outputBuffer )
{
	...
	else if (basicSize == 126)
	{
		...
		u_short len = ntohs( *(u_short*)inputBuffer.getBuffer(2) );
		payloadSize = len;
	...
	else if (basicSize == 127)
	{
		...
		u_long len = ntohl( *(u_long*) inputBuffer.getBuffer(2+4) ); //TODO.
		payloadSize = len;
	...
	if (isMasked)
	{
		memcpy(masks, inputBuffer.getBuffer(payloadOffset), 4);
		payloadOffset += 4;
	}
	...
	{
		RecyclableBuffer controlFrameReply;
	...
	Buffer& targetBuffer = shouldDoBuffering ? websocketContext.bufferedContent : outputBuffer;
	...
	if (shouldDoBuffering)
	{
		if (isFIN)
		{
			if (!outputBuffer.append(websocketContext.bufferedContent))
				return DecodeResult::Failure;

			websocketContext.bufferedContent.clearBytes();
...
DecodeResult::Type WebsocketProtocol::tryDecodeHanshakeData( ProtocolContext& context, Buffer& outputBuffer )
{
	...
	for (int i=0;i<inputBuffer.getDataSize();i++)
	{
		...
		if (strncmp(httpEnd, inputBuffer.getBuffer(i), 4) != 0)
			continue;

		if (!isClientSide)
		{
			inputBuffer.getBuffer()[i] = '\0';
			Buffer request;			
			request.assign(inputBuffer, i+1);
